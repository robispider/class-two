...
export default class QuestionGenerator {
    constructor(maxNumber, performanceTracker) {
        this.maxNumber = maxNumber; // Maximum number for factors (e.g., 10)
    ...
    generateAdaptiveQuestion(fixedB = null, stage = 1) {
        ...
        // If in practice mode, filter problematic problems to match fixedB
        if (fixedB !== null) {
        ...
        let a, b, target;
        let tries = 0;
        const maxFactor = 10; // Maximum value for any factor
        const maxTarget = 50; // Maximum target value (5 * 10)

        do {
            if (problematic.length > 0 && Math.random() < prob) {
                // Select from problematic problems
                ...
                // In practice mode, b is fixed to allowedTables[0]
                if (fixedB !== null) {
                    b = fixedB;
                    a = Math.floor(Math.random() * maxFactor) + 1;
                } else {
                    // In stage mode, one factor must be from allowedTables
                    if (Math.random() < 0.5) {
            ...
            (a === this.lastA && b === this.lastB || target === this.lastTarget || 
             a > maxFactor || b > maxFactor || target > maxTarget) && tries < 5
        );
        ...
        console.debug("Generated questionData:", questionData);
        return questionData;
    }
 generateSpecificQuestion(a, b) {
        ...
        // Get all factors of the target, filtering by the max allowed factor.
        const factors = this.getFactors(target).filter(f => f <= maxFactor);

        const questionData = { a, b, target, factors };
        console.debug("Generated specific questionData:", questionData);
        return questionData;
    }
    ...
    generateOptions(correct, a, b) {
        ...
        const maxTarget = 50; // Maximum target value
        const candidates = [
            a * (b + 1), 
            a * (b - 1), 
            (a + 1) * b, 
            ...
            correct - 2, 
            correct + 10
        ].filter(p => p > 0 && p <= maxTarget); // Ensure options are positive and within maxTarget
    ...
    // Set allowedTables for use in question generation
